...
def global_ssim(feat, fg_mask, alpa=1, beta=1, gamma=1):
    ...
    # avg pooling to 1*1
    bg_pooling = torch.sum(bg.view(N, C, -1), dim=2, keepdim=True).div(
        bg_mask_sum)  # each channel mean  == channel avg pooling
    ...
    fg_bg_gobal_conv = torch.matmul((bg_pooling - bg_distribution_mu).permute(0, 2, 1),
                                    (fg_pooling - fg_distribution_mu)) / (C - 1)
