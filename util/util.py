...
def gradient(input_tensor, direction):
    ...
    smooth_kernel_x = torch.reshape(torch.Tensor([[0., 0.], [-1., 1.]]), (1, 1, 2, 2)).repeat(1, c, 1, 1).to(
        input_tensor.get_device())
...
def gradient_sobel(input_tensor, direction):
    ...
    smooth_kernel_x = torch.reshape(torch.Tensor([[-1., 0., 1], [-2., 0, 2.], [-1, 0, 1]]), (1, 1, 3, 3)).to(
        input_tensor.get_device())
    smooth_kernel_y = smooth_kernel_x.permute(0, 1, 3, 2)
