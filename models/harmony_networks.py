...
class CNNHTGenerator(nn.Module):
    def __init__(self, opt=None):
        ...
        dim = 256
        self.enc = ContentEncoder(opt.n_downsample, 0, opt.input_nc + 1, dim, opt.ngf, 'in', opt.activ,
        ...
        if opt.use_two_dec:
            self.dec = ContentDecoder(opt.n_downsample, 0, dim, opt.output_nc, opt.ngf, 'ln', opt.activ,
                                      pad_type=opt.pad_type)
...
class DHTGenerator(nn.Module):
    ...
    def forward(self, inputs=None, image=None, pixel_pos=None, patch_pos=None, mask_r=None, mask=None, layers=[],
                encode_only=False):
    ...
    def __init__(self, input_dim, output_dim, kernel_size, stride,
        ...
        if norm == 'sn':
            self.conv = SpectralNorm(
                nn.ConvTranspose2d(input_dim, output_dim, kernel_size, stride, padding=padding, bias=self.use_bias))
        else:
            self.conv = nn.ConvTranspose2d(input_dim, output_dim, kernel_size, stride, padding=padding,
                                           bias=self.use_bias)
