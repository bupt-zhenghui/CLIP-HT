...
class RealDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        target_path = self.image_paths[index].replace('composite_images', 'real')
        target_path = path
        # target_path = target_path.replace(('_'+name_parts[-2]+'_'+name_parts[-1]),'.png')

        name_part = name_parts[-3].split('/')
        ...
        if np.random.rand() > 0.5 and self.isTrain:
            comp, mask = tf.hflip(comp), tf.hflip(mask)
